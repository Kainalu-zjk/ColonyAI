...
async def get_service_status():
    ...
    try:
        ...
        if redis_client is not None:
            ...
            status["redis"] = {"status": "warning", "message": "Redis unavailable — running in file-only fallback mode"}
    except Exception as e:
        logger.error(f"Redis connection failed: {str(e)}")
        status["redis"] = {"status": "error", "message": f"Redis unavailable: {str(e)}. Using file-only mode."}
