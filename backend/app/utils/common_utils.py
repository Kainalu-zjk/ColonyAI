...
def get_config_template(comp_template: CompTemplate = CompTemplate.CHINA) -> dict:
    ...
    """
    _TEMPLATE_MAP = {
        CompTemplate.CHINA: "md_template.toml",
        CompTemplate.AMERICAN: "md_template_en.toml",
        CompTemplate.HUASHU: "md_template_huashu.toml",
        CompTemplate.HUAWEI: "md_template_huawei.toml",
    }
    filename = _TEMPLATE_MAP.get(comp_template)
    if filename:
        return load_toml(os.path.join("app", "config", filename))
    return {}


def load_toml(path: str) -> dict:
    """加载 TOML 配置文件，自动补齐 key 别名确保兼容性。
    ...
    with open(path, "rb") as f:
        data = tomllib.load(f)
    # 兼容 key 别名：旧模板用的是 assumption/sign/sensitivity
    # flows.py 需要的是 modelAssumption/symbol/sensitivity_analysis/judge/eda
    _ALIASES = {
    ...
    # 确保 judge 和 eda 存在（如果模板没有就用默认值）
    if "judge" not in data:
        data["judge"] = data.get("sensitivity_analysis", "# 模型评价（默认）")
    if "eda" not in data:
        data["eda"] = "# 探索性数据分析（默认）"
    # 兼容中文模板（使用 coreQues 代替单独的 ques1/ques2/ques3）
    # get_writer_prompt 会访问 config_template["ques1"] 等键
    # 如果模板有 coreQues 但没有单独的 ques1/ques2/ques3，自动生成
    if "coreQues" in data:
        core_ques = data["coreQues"]
        for i in range(1, 7):
            qkey = f"ques{i}"
            if qkey not in data:
                data[qkey] = core_ques.replace("{num}", str(i))
    return data
