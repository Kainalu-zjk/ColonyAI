...
class OpenAlexScholar:
    ...
    def papers_to_str(self, papers: List[Dict[str, Any]], style: str = "apa") -> str:
        """将文献列表转换为可读字符串。

        Args:
            papers: 论文列表。
            style: 引用风格 ("apa" 或 "gb7714")。
    ...
    def _format_citation(self, work: Dict[str, Any], style: str = "apa") -> str:
        ...
        # Get journal name from primary_location
        ...
        if style == "apa":
            ...
            if len(authors) > 3:
                ...
                last = authors.pop()
                authors_str = ", ".join(authors) + f", & {last}"
            elif len(authors) == 1:
            ...
            if first_page and last_page:
                ...
                citation += f": {first_page}."
            if doi:
                citation += f" doi:{doi.replace('https://doi.org/', '')}."
            return citation
